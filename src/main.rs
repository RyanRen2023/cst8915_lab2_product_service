...
async fn main() {
    ...
    // 定义根路径 "/" 路由。
    let root = warp::path::end().map(|| warp::reply::html("Welcome to the Warp web server!"));

    // 将根路径和 "/products" 路由合并。
    let routes = root.or(products);
    ...
    warp::serve(routes).run(([0, 0, 0, 0], 3030)).await; // Await the server to ensure it's running asynchronously.
}
